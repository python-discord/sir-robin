...
class Levels(commands.Cog):
    ...
    async def cog_load(self) -> None:
        ...
        if await self.levels_cache.length() == 0:
            ...
            init_threshold_dict = {role: 0 for role in shuffled_roles}
    ...
    async def _cycle_rules_task(self):
    ...
    async def _calculate_point_thresholds_task(self):
        ...
        new_levels = dict(zip(levels.keys(), thresholds))
    ...
    async def _update_points(self, user_id: int, points: int):
    ...
    async def _update_role_assignment(self, user_id: int):
    ...
    async def shuffle_role_order(self, ctx: commands.Context):
        ...
        updated_ordering = dict(zip(role_order, thresholds))
